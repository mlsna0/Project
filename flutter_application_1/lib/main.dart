...
import 'package:flutter_application_1/block_unit.dart';
import 'package:flutter_application_1/coordinate.dart';
import 'package:flutter_application_1/block_unit_manager.dart';
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void initTable() {
    table = <List<BlockUnit>>[];
    for (int row = 0; row < 4; row++) {
      List<BlockUnit> list = <BlockUnit>[];
  ...
  List<BlockUnit> listVertical = <BlockUnit>[];
  ...
  List<Coordinate> getBlockEmptyList() {
    List<Coordinate> listBlockUnitEmpty = <Coordinate>[];
    for (int row = 0; row < 4; row++) {
      for (int col = 0; col < 4; col++) {
        if (table[row][col].value == BLOCK_VALUE_NONE) {
  ...
  style: ElevatedButton.styleFrom(
    primary: Color(0xff8f7a66), // สีของปุ่ม
  ),
  onPressed: () {
    restart();
  },
